...
class NeuralController : public controller_interface::ControllerInterface {
 ...
 protected:
  ...
  static constexpr int kBaseObservationSize = 3                /* base link angular velocity */
  ...
  // Gait policies append the 12-dim reference joint offset they track, right after
  // the previous action, making the frame kBaseObservationSize + kActionSize.
  static constexpr int kGaitReferenceIdx = kBaseObservationSize;
  static constexpr int kGaitObservationSize = kBaseObservationSize + kActionSize;
  static constexpr int kGravityZIndx = 5;  // Index of gravity z component in the observation
  /* ----------------------------------------------- */

  // Frame size of this policy: kBaseObservationSize, or kGaitObservationSize when
  // the deploy JSON declares a gait reference. Read from the JSON's
  // "single_observation_size" so one controller serves both policy families.
  int single_observation_size_ = kBaseObservationSize;

  // Gait reference tables, empty (and unused) for non-gait policies.
  GaitReference gait_;
